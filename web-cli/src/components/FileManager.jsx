...
const handlePreview = async (file) => {
  ...
  const handleSync = async (e) => {
    ...
    for (let i = 0; i < fileList.length; i++) {
      localFileNames.add(fileList[i].name);
    }

    const filesToDelete = files.filter(serverFile => 
      !localFileNames.has(`${serverFile.title}${serverFile.extension}`)
    );

    if (filesToDelete.length > 0) {
      const msg = `Syncing will delete ${filesToDelete.length} file(s) from your server that aren't in this local folder. Proceed?`;
      if (!confirm(msg)) {
        e.target.value = ''; 
        return;
      }
    ...
    for (const file of filesToDelete) {
      try {
        ...
        console.error(`Failed to delete ${file.title}:`, err);
      }
    }
    ...
    if (failedUploads > 0) {
      alert(`Sync finished with ${failedUploads} error(s). Check console.`);
    }
    await loadFiles();
    e.target.value = '';
  };
    ...
    >
      ...
      <div className="toolbar">
        ...
        <div>
          <label className="btn">
            Sync Folder
            <input type="file" hidden webkitdirectory="true" onChange={handleSync} />
          </label>
        </div>
